...
// 状态变化回调函数
void on_state_change(vad_state_t old_state, vad_state_t new_state,
                     void *user_data) {
...
int main(int argc, char *argv[]) {
  ...
  printf("   - 帧大小: %d samples (%.1fms)\n", hop_size,
         (float)hop_size * 1000 / sample_rate);
  printf("   - 阈值: %.2f\n", threshold);
  ...
  for (int i = 0; i < scenario_count; i++) {
    ...
    // VAD处理
    if (ten_vad_process(vad_handle, audio_buffer, hop_size, &probability,
                        &vad_flag) == 0) {
      // 状态机处理
      vad_state_t current_state =
          vad_state_machine_process(state_machine, vad_flag, probability);

      printf("[%03d] VAD: %d (%.3f) | 状态: %s | 持续: %.1fms\n", i, vad_flag,
             probability, vad_state_get_name(current_state),
